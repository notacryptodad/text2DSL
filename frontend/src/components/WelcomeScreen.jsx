import { Zap, Database, Brain, CheckCircle, ArrowRight, ChevronDown, ChevronUp } from 'lucide-react'
...
function WelcomeScreen({ onGetStarted }) {
  ...
  return (
    <div className="flex items-center justify-center h-full p-6">
      <div className={`text-center transition-all duration-300 ${isCollapsed ? 'max-w-xs' : 'max-w-2xl'}`}>
        {/* Collapse Toggle */}
        <button
        ...
        {isCollapsed ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-primary-400 to-primary-600 rounded-xl mb-3">
              <Zap className="w-6 h-6 text-white" />
            </div>
            <p className="text-sm font-medium text-gray-900 dark:text-white">Text2DSL</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Click to expand</p>
          </div>
        ) : (
          /* Expanded State */
          <>
