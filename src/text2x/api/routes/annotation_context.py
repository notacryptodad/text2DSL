...
def format_context_as_prompt(context: Dict[str, Any]) -> str:
    ...
    if fk_out or fk_in:
        ...
        for fk in fk_out:
            lines.append(f"  → {context['table_name']}.{fk['column']} references {fk['references_table']}.{fk['references_column']}")
        for fk in fk_in:
            lines.append(f"  ← {fk['from_table']}.{fk['from_column']} references {context['table_name']}.{fk['to_column']}")
        lines.append("")
